...
public class Filter {
  ...
  public void constructFilter() {
    ...
    if (!(this.filterQuery.contains("AND") || this.filterQuery.contains("OR"))) {
      ...
      List<Join> joinPath = this.joinBuilder.getPath(this.filterTableName, this.entityTableName, this.entityPK); // TODO:
                                                                                                                 // could
                                                                                                                 // optimize
                                                                                                                 // by
                                                                                                                 // building
      ...
      if (joinPath.isEmpty()) { // Filter on the entity table
        ...
        for (ForeignKey fk : this.generator.getEntityTable().getForeignKeys()) {
          if (fk.getFromField().equals("integer_id_alias")) {
            ...
            String union_intersect_template = "SELECT MAPPINGENTITYKEY AS MAPPINGENTITYKEY FROM FILTERPRESELECTNAME";
  ...
  public void buildLeftRightFilters() {
    ...
    // Construct nested Filter objects for left and right filters (adding '_0' to
    // ids for left and '_1' to ids for right filters)
  ...
  public void setVariablesFromChildren() { // Concatenate nested filter values
    if (this.leftFilter != null & this.rightFilter != null) { // Check to see that we have left and right child Filters
      ...
      this.unionIntersect = "(" + this.leftFilter.getUnionIntersect() + " " + this.operator + " "
          + this.rightFilter.getUnionIntersect() + ")";
  ...
  public void setIncludeCountQuery() {
    if (this.isRoot & this.leftFilter == null & this.rightFilter == null) {
      // Don't need to add mapping table preselect statements and union/intersect
      // statements if the query isn't nested
  ...
  public String replaceKeywords(String template) { // Helper function for replacing constructed string variables with
                                                   // supplied template
    return template
  ...
  public void setJoinString(List<Join> joinPath) { // Builds out join statements from JoinPath
    ...
    for (Join join : joinPath) {
      if (join != joinPath.get(joinPath.size() - 1)) { // Don't need final path since it will always be entity table
                                                       // since we have a mapping table before it
