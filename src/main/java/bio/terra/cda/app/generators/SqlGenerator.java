...
public class SqlGenerator {
    ...
    public SqlGenerator(String qualifiedTable, Query rootQuery, String version, boolean filesQuery)
        ...
        this.fileTable = dotPos == -1 ? qualifiedTable.replace("Subjects", TableSchema.FILES_COLUMN)
                : qualifiedTable.substring(dotPos + 1).replace("Subjects",
                        TableSchema.FILES_COLUMN);
        this.tableDefinition = TableSchema.getSchema(version);
        this.dataSetInfo = new DataSetInfo.DataSetInfoBuilder()
                .addTableSchema(version, this.tableDefinition).build();
    ...
    public SqlGenerator(String qualifiedTable, Query rootQuery, String version, boolean filesQuery,
            ParameterBuilder parameterBuilder, ViewListBuilder<View, ViewBuilder> viewListBuilder)
            throws IOException {
    ...
    protected void initializeBuilders() {
        ...
        this.viewListBuilder =
                new ViewListBuilder<>(ViewBuilder.class, this.dataSetInfo, this.project);
        this.unnestBuilder = new UnnestBuilder(this.queryFieldBuilder, this.viewListBuilder,
                this.dataSetInfo, this.entityTable, project);
    ...
    protected QueryContext buildQueryContext(TableInfo entityTable, boolean filesQuery,
            boolean subQuery) {
        return new QueryContext(table, project).setFilesQuery(filesQuery).setTableInfo(entityTable)
                .setIncludeSelect(!subQuery).setQueryFieldBuilder(queryFieldBuilder)
                .setSelectBuilder(selectBuilder).setUnnestBuilder(unnestBuilder)
                .setPartitionBuilder(partitionBuilder).setParameterBuilder(parameterBuilder)
                .setOrderByBuilder(orderByBuilder).setViewListBuilder(viewListBuilder);
    }
    ...
    public QueryJobConfiguration.Builder generate() throws IllegalArgumentException {
        ...
        this.parameterBuilder.getParameterValueMap()
                .forEach(queryJobConfigBuilder::addNamedParameter);
    ...
    protected String resultsQuery(Query query, String tableOrSubClause, boolean subQuery,
            QueryContext ctx, boolean hasSubClause) {
        ...
        TableInfo startTable =
                Objects.isNull(entityPath) || entityPath.length == 0 ? this.entityTable
        ...
        if (query.getWhere().getNodeType() == Operator.NodeTypeEnum.SUBQUERY) {
            ...
            left.setWhere(query.getWhere().getLeft());
            right.setWhere(query.getWhere().getRight());


            return resultsQuery(left,
                    String.format("(%s) as %s",
                            sql(tableOrSubClause, right, true, hasSubClause, true),
                            startTable.getTableAlias(this.dataSetInfo)),
                    subQuery, buildQueryContext(ctx.getTableInfo(), filesQuery, subQuery), true);
        }
        ...
        if (filesQuery) {
            ctx.addUnnests(this.unnestBuilder.fromRelationshipPath(pathToFile,
                    SqlUtil.JoinType.INNER, false));
        }
        ...
        if (subQuery) {
            return SqlTemplate.regularQuery(
                    String.format("%s.*", startTable.getTableAlias(this.dataSetInfo)), fromString,
                    condition, ctx.getOrderBys().stream().map(OrderBy::toString)
                            .collect(Collectors.joining(", ")));
        }
    ...
    protected Stream<String> getPartitionByFields(QueryContext ctx) {
        return Stream.concat(
                ...
                ctx.getPartitions().stream().map(Partition::toString)).distinct();
    }
    ...
    protected Stream<String> getSelectsFromEntity(QueryContext ctx, String prefix,
        ...
        if (this.entityTable.getType().equals(TableInfo.TableInfoTypeEnum.NESTED)
            ...
            if (Objects
                    .isNull(this.dataSetInfo.getSchemaDefinitionByFieldName(entityPartitionKey))) {
                entityPartitionKey =
                        DataSetInfo.getNewNameForDuplicate(this.dataSetInfo.getKnownAliases(),
                                entityPartitionKey, this.entityTable.getTableName());
            }

            idSelects = Stream.concat(Stream.of(String.format("%s AS %s",
                    ...
                    Arrays.stream(path).map(tableRelationship -> {
                        ...
                        if (Objects.isNull(
                            ...
                            partitionKey = DataSetInfo.getNewNameForDuplicate(
                                    this.dataSetInfo.getKnownAliases(), partitionKey,
            ...
            if (path.length > 0) {
                ...
                ctx.addPartitions(
                        Stream.of(this.partitionBuilder.of(this.entityTable.getPartitionKey(),
    ...
    protected Stream<String> combinedSelects(QueryContext ctx, String prefix, boolean skipExcludes,
            Stream<String> idSelects) {
        ...
        return Stream.concat(Arrays
                .stream(ctx.getFilesQuery()
                        ? this.dataSetInfo.getTableInfo(TableSchema.FILE_PREFIX)
                                .getSchemaDefinitions()
                        : this.entityTable.getSchemaDefinitions())
                .filter(definition -> !(ctx.getFilesQuery()
                ...
                .map(definition -> {
                    ...
                    if (definition.getMode().equals(Field.Mode.REPEATED.toString())
                            && ctx.getOrderBys().stream().anyMatch(
                                    ob -> ob.getFieldName().equals(definition.getName()))) {
                        fieldSelect = this.dataSetInfo.getTableInfo(definition.getName())
                                .getTableAlias(this.dataSetInfo);
                    }
    ...
    protected String getWithStatement() {
        return String.format("WITH %s ", this.viewListBuilder.build().stream().map(View::toString)
                .collect(Collectors.joining(", ")));
    }
}
