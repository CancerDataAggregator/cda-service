...
public class FileSqlGenerator extends SqlGenerator {
  ...
  protected void initializeEntityFields() {
      ...
      this.entityTable = startsWithFile
  ...
  protected String sql(
    ...
    return startsWithFile
            ...
            : startsWithEntityQuery(tableOrSubClause, query, subQuery, hasSubClause, ignoreWith);
  }

  private String startsWithFileQuery(
          String tableOrSubClause,
          Query query,
          boolean subQuery,
          boolean hasSubClause,
      ...
      Arrays.stream(this.entityTable.getSchemaDefinitions())
              ...
              .forEach(schemaDefinition -> {
                  List<Unnest> unnestList = this.unnestBuilder.fromRelationshipPath(
                  ...
                  Arrays.stream(schemaDefinition.getForeignKeys())
                          .forEach(foreignKey -> {
  ...
  private String startsWithEntityQuery(String tableOrSubClause,
                                       Query query,
                                       boolean subQuery,
                                       boolean hasSubClause,
                                       boolean ignoreWith) {
  ...
  protected Stream<String> getSelectsFromEntity(
    ...
    List<String> idSelects = startsWithFile
            ? buildIdSelectsStartsWithFile(ctx)
            : buildIdSelects(ctx);
              ...
              .forEach(schemaDefinition -> {
                  ...
                  ctx.addUnnests(
                          this.unnestBuilder.fromRelationshipPath(
                                  ...
                                  SqlUtil.JoinType.LEFT,
                                  false));
                  ctx.addPartitions(Stream.of(this.partitionBuilder.of(
