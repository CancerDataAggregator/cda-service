...
public class CountsSqlGenerator extends SqlGenerator {
    ...
    private String sql(String tableOrSubClause, Query query) {
        ...
        sb.append(String.format("  LEFT OUTER JOIN (\n"
                ...
                + "    GROUP BY\n"
                + "      _File_identifier.system\n"
                + "  ) AS top_level_file on top_level_file.system = identifiers.system\n", topLevelFrom, whereClause));

        var rsFrom = Stream.concat(
                fromClause.get(), getUnnestsFromParts(new String[] {"ResearchSubject", "identifier"}, true)
        ).distinct().collect(Collectors.joining(",\n"));
        sb.append(String.format("  LEFT OUTER JOIN (\n"
                + "    SELECT\n"
        ...
        var rsFileFrom = Stream.concat(
                fromClause.get(), getUnnestsFromParts(new String[]{ "ResearchSubject", "File", "identifier"}, true)
        ).distinct().collect(Collectors.joining(",\n"));
        sb.append(String.format("  LEFT OUTER JOIN (\n"
                + "    SELECT\n"
        ...
        var specimenFrom = Stream.concat(
                fromClause.get(), getUnnestsFromParts(new String[] { "ResearchSubject", "Specimen", "identifier"}, true)
        ).distinct().collect(Collectors.joining(",\n"));
        sb.append(String.format("  LEFT OUTER JOIN (\n"
                + "    SELECT\n"
                ...
                + "    GROUP BY\n"
                + "      _ResearchSubject_Specimen_identifier.system\n"
                + "  ) AS specimen_count ON specimen_count.system = identifiers.system\n", specimenFrom, whereClause));

        var specimenFileFrom = Stream.concat(
                fromClause.get(), getUnnestsFromParts(new String[] { "ResearchSubject", "Specimen", "File", "identifier"}, true)
        ).distinct().collect(Collectors.joining(",\n"));
        sb.append(String.format("  LEFT OUTER JOIN (\n"
                + "    SELECT\n"
        ...
        var identFrom = Stream.concat(
                fromClause.get(), getUnnestsFromParts(new String[] { "identifier"}, true)
        ).distinct().collect(Collectors.joining(",\n"));
        sb.append(String.format("  LEFT OUTER JOIN (\n"
                + "    SELECT\n"
                + "      _identifier.system,\n"
