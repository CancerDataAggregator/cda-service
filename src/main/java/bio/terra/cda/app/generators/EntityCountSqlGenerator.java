...
public class EntityCountSqlGenerator extends SqlGenerator {
  ...
  protected String sql(
    ...
    //      withStatement = String.format("%s, %s as (%s)", getWithStatement(), tableAlias,
    // viewSql);
    //    } else {
    ...
    return subQuery
        ? viewSql
        : String.format("%s select %s", getWithStatement(), getCountSelects(tableAlias));
  }

  protected void addGroupedCountViews(String tableAlias) {
    groupedCountFields.forEach(col -> addEachGroupedCountView(col, tableAlias));
  }
  ...
  protected void addEachGroupedCountView(ColumnDefinition col, String fromTableAlias) {
    ...
    String groupedCountInnerView =
        String.format(
            ...
            fieldName, this.entityTable.getPrimaryKeysAlias().get(0), fromTableAlias);
    ...
    viewBuilder
        ...
        .addSelect(
            new Select("row_to_json(subq)", String.format(viewSelectFormatString, fieldName)))
  ...
  protected String getCountSelects(String tableAlias) {
    ...
    String groupedFormatString = "(SELECT array_agg(json_%1$s) from %1$s_count) as %1$s";
