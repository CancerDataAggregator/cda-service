    ...
    return this.countFields.stream()
        ...
        .map(
            countByField -> {
              ...
              String name = Objects.nonNull(countByField.getAlias()) ? countByField.getAlias() : countByField.getField();

              String fieldToUse = countByField.getField();

              if (Objects.isNull(this.dataSetInfo.getSchemaDefinitionByFieldName(countByField.getField()))) {
                fieldToUse = DataSetInfo.getNewNameForDuplicate(this.dataSetInfo.getKnownAliases(), countByField.getField(), tableToUse.getAdjustedTableName());
              }
    ...
    return this.countFields.stream()
        .map(
            countByField -> {
              ...
              TableSchema.SchemaDefinition definition = this.dataSetInfo.getSchemaDefinitionByFieldName(fieldToUse);
              ...
              if (definition.getMode().equals(Field.Mode.REPEATED.toString())) {
                ...
                return String.format(
                        "%1$s.%2$s AS %3$s", tableToUse.getTableAlias(this.dataSetInfo), definition.getName(), definition.getAlias());
              }
...
//                    TableInfo countByTable = this.dataSetInfo.getTableInfo(countByField.getTable());
//                    ctx.addUnnests(
//                            this.unnestBuilder.fromRelationshipPath(
//                                    this.entityTable.getPathToTable(countByTable), SqlUtil.JoinType.LEFT, true));
//                    ctx.addPartitions(
//                            this.partitionBuilder.fromRelationshipPath(
...
//                                    .filter(def -> def.getName().equals(countByField.getField()))
//                                    .findFirst()
//                                    .orElseThrow();
//
//                    if (countByTableField.getMode().equals(Field.Mode.REPEATED.toString())) {
//                      TableInfo countByArrayTable = this.dataSetInfo.getTableInfoFromField(countByTableField.getName());
//
//                      if (Objects.isNull(countByArrayTable)) {
//                        countByArrayTable = this.dataSetInfo.getTableInfoFromField(countByTableField.getAlias());
//                      }
//                      ctx.addUnnests(
//                              this.unnestBuilder.fromRelationshipPath(
...
//              }
  ...
  protected List<CountByField> getCountByFields() {
    TableInfo countFieldsTable = filesQuery
