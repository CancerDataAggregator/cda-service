...
public class ViewBuilder {
  ...
  public ViewBuilder(DataSetInfo dataSetInfo, String project) {
  ...
  public ViewBuilder addUnnests(Stream<Unnest> unnestStream) {
    this.unnestList =
        Stream.concat(this.unnestList.stream(), unnestStream).collect(Collectors.toList());
    return this;
  }
  ...
  public View build() {
    String fromClause =
        Stream.concat(
                Stream.of(
                    String.format(
                        "%s.%s AS %s",
                        project, table.getTableName(), table.getTableAlias(this.dataSetInfo))),
    ...
    return new View(
        this.viewName, this.viewType, select, fromClause, this.whereClause, this.includeAlias);
  }
}
