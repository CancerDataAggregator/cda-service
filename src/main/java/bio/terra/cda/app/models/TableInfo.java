...
public class TableInfo {
  ...
  //    return
  // Arrays.stream(this.primaryKeys).map(dataSetInfo.getColumnDefinitionByFieldName).collect(Collectors.toList());
  ...
  public static class TableInfoBuilder {
    ...
    public TableInfoBuilder setColumnDefinitions(List<ColumnDefinition> columnDefinitions) {
    ...
    protected List<ColumnDefinition> getColumnDefinitionForPrimaryKeys() {
      return Arrays.stream(columnDefinitions)
          .filter(cd -> primaryKeys.contains(cd.getName()))
          .collect(Collectors.toList());
    }
