...
public class TableInfo {
    ...
    private TableInfo(String tableName, String adjustedTableName, TableInfoTypeEnum type,
            TableSchema.SchemaDefinition[] schemaDefinitions, String partitionKey) {
    ...
    public void addRelationship(TableRelationship tableRelationship) {
        Optional<TableRelationship> optRelationship = relationships.stream()
                .filter(rel -> rel.getDestinationTableInfo().getTableName()
    ...
    public String getPartitionKeyFullName(DataSetInfo dataSetInfo) {
        ...
        if (this.getType().equals(TableInfoTypeEnum.TABLE)) {
            table = dataSetInfo.getKnownAliases().getOrDefault(this.tableName, this.tableName)
    ...
    public TableInfo getSuperTableInfo() {
        if (this.getType().equals(TableInfoTypeEnum.TABLE)) {
            ...
            while (!current.getType().equals(TableInfoTypeEnum.TABLE)) {
                TableRelationship parent = current.getRelationships().stream()
                        .filter(TableRelationship::isParent).findFirst().orElseThrow();
    ...
    public TableRelationship[] getPathToTable(TableInfo tableInfo, boolean noParent) {
        ...
        LinkedList<Tuple<TableRelationship[], TableRelationship>> queue = this.getRelationships()
                .stream().filter(tableRelationship -> (!tableRelationship.isParent() || !noParent))
        ...
        while (queue.size() > 0) {
            ...
            if (foreignKey.stream().anyMatch(
                    fk -> Objects.nonNull(fk.getLocation()) && fk.getLocation().length() > 0)) {
                continue;
            }
            ...
            if (tableRelationship.getDestinationTableInfo().getAdjustedTableName()
            ...
            List<Tuple<TableRelationship[], TableRelationship>> moreRelationships = nextTable
                    .getRelationships().stream()
                    .filter(tr -> Objects.isNull(
                            visited.get(tr.getDestinationTableInfo().getAdjustedTableName()))
                            && (!tableRelationship.isParent() || !noParent))
                    .map(tr -> Tuple.of(relArray, tr)).collect(Collectors.toList());
    ...
    public static TableInfo of(String tableName, TableInfoTypeEnum type,
            TableSchema.SchemaDefinition[] schemaDefinitions, String partitionKey) {
        return new TableInfoBuilder().setTableName(tableName).setType(type)
                .setSchemaDefinitions(Arrays.asList(schemaDefinitions))
                .setPartitionKey(partitionKey).build();
    }

    public static TableInfo of(String tableName, String adjustedTableName, TableInfoTypeEnum type,
            TableSchema.SchemaDefinition[] schemaDefinitions, String partitionKey) {
        return new TableInfoBuilder().setTableName(tableName)
                .setAdjustedTableName(adjustedTableName).setType(type)
                .setSchemaDefinitions(Arrays.asList(schemaDefinitions))
                .setPartitionKey(partitionKey).build();
    }
    ...
    public static class TableInfoBuilder {
        ...
        public TableInfo build() {
            return new TableInfo(this.tableName, this.adjustedTableName, this.type,
                    this.schemaDefinitions, this.partitionKey);
        }
