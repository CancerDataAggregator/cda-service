...
public class In extends BasicOperator {
  ...
  public String buildQuery(QueryContext ctx) {
    ...
    if (right.contains("\"") || right.contains("'")) {
      ...
      paramNames =
          values.stream()
              .map(value -> parameterBuilder.addParameterValue(type, value))
              .map(name -> String.format("UPPER( %s )", name))
              .collect(Collectors.toList());
    } else {
      paramNames =
          Arrays.stream(right.split(","))
              .map(value -> parameterBuilder.addParameterValue(type, value.trim()))
              .collect(Collectors.toList());
    }
    //    parameterName = parameterBuilder.addParameterValue(queryField, Strings.join(values, ','));
    right = String.format("( %s )", Strings.join(paramNames, ','));
