...
public class QueryDeserializer extends JsonDeserializer<Query> {
  ...
  public Query deserialize(JsonParser p, DeserializationContext ctxt)
    ...
    if (Objects.nonNull(selectNode) && selectNode.isArray()) {
      try {
        query.setSelect(
            new InnerQueryDeserializer<>().buildNodeOrColumn(selectNode, codec, Select.class));
    ...
    if (Objects.nonNull(orderByNode) && orderByNode.isArray()) {
      try {
        query.setOrderBy(
            new InnerQueryDeserializer<>().buildNodeOrColumn(orderByNode, codec, OrderBy.class));
      } catch (JsonProcessingException exception) {
        throw exception;
      }
    }
  ...
  private static class InnerQueryDeserializer<T extends OperatorArrayInner> {
     ...
     */
    public List<T> buildNodeOrColumn(
        JsonNode arrayJsonNode, ObjectCodec codec, Class<? extends T> clz)
        throws JsonProcessingException {
      ...
      for (JsonNode operator : arrayJsonNode) {
        ...
        if (Objects.nonNull(newOperator)) {
          if (operator.hasNonNull("node_type")) {
            ...
            ((ListOperator) newOperator)
                .setOperator((BasicOperator) codec.treeToValue(operator, Operator.class));
            ...
            Column column = (Column) new Column().setValue(operator.textValue());
