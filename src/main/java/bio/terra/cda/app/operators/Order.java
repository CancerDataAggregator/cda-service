    ...
    try {
      Arrays.stream(getValue().split(","))
          ...
          .forEach(
              orderBy -> {
                var parts =
                    Arrays.stream(orderBy.split("\\s+")).map(String::trim).toArray(String[]::new);
                ...
                ctx.addUnnests(
                    SqlUtil.getUnnestsFromParts(
                        ctx.getTable(), columnParts, (tmpGetMode.equals("REPEATED"))));
              });
                ...
                ctx.addOrderBy(
                    String.format(
                        ...
                        columnParts.length == 1
                            ? ctx.getTable()
                            : SqlUtil.getAlias(columnParts.length - 2, columnParts),
