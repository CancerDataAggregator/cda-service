...
public class SelectValues extends BasicOperator {
  ...
  public void addUnnests(QueryContext ctx) {
    ctx.addUnnests(Arrays.stream(getValue().split(","))
            .map(String::trim)
            .filter(select -> {
                var tmp = (ctx.getFilesQuery()
                        ? ctx.getFileTableSchemaMap()
                        : ctx.getTableSchemaMap()).get(select);
                return !tmp.getMode().equals("REPEATED");
            })
            .flatMap(
                    select -> {
                        var isFileField = select.toLowerCase().startsWith("file.");
                        var value = isFileField
                                ? select.substring(select.indexOf(".") + 1)
                                : select;
  ...
  private void addPartitions(QueryContext ctx) {
    ctx.addPartitions(Arrays.stream(getValue().split(","))
            .map(String::trim)
            .filter(
                    select -> {
                        var tmp = (ctx.getFilesQuery()
            ...
            .map(
                    select -> {
                      ...
                      var tmp = (ctx.getFilesQuery()
                              ? ctx.getFileTableSchemaMap()
                              : ctx.getTableSchemaMap()).get(select);
                      if (Arrays.asList(parts).contains("identifier") && !parts[parts.length - 1].equals("identifier")) {
