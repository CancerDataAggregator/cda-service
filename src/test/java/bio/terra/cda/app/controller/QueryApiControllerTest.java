...
class QueryApiControllerTest {
  ...
  void uniqueValuesTest() throws Exception {
    ...
    var expected =
        "SELECT DISTINCT sex FROM subject,  subject_identifier AS subject_identifier  WHERE system = 'GDC' AND  subject.id = subject_identifier.subject_id  ORDER BY sex  LIMIT 100";
    var result =
        mvc.perform(
