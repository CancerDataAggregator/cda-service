...
class QueryApiControllerTest {
    ...
    void uniqueValuesTest() throws Exception {
        ...
        var result = mvc.perform(post("/api/v1/unique-values/{version}", version)
                .param("system", system).param("table", table).param("count", String.valueOf(count))
                .contentType(MediaType.valueOf("text/plain")).content(body)
                .accept(MediaType.APPLICATION_JSON)).andReturn();
        var response = objectMapper.readValue(result.getResponse().getContentAsString(),
                QueryCreatedData.class);
