  ...
  public static Stream<Arguments> queryData() {
    return Stream.of(
        Arguments.of(
            ...
            QUALIFIED_TABLE,
            TABLE,
            "with specimen_files as (SELECT results.* EXCEPT(rn) FROM (SELECT ROW_NUMBER() OVER (PARTITION BY File.id, Subject.id, _ResearchSubject.id, _Specimen.id) as rn, File.id AS file_id, File.identifier AS file_identifier, File.label AS label, File.data_category AS data_category, File.data_type AS data_type, File.file_format AS file_format, File.associated_project AS file_associated_project, File.drs_uri AS drs_uri, File.byte_size AS byte_size, File.checksum AS checksum, File.data_modality AS data_modality, File.imaging_modality AS imaging_modality, File.dbgap_accession_number AS dbgap_accession_number, File.imaging_series AS imaging_series, _Specimen.id AS specimen_id, _ResearchSubject.id AS researchsubject_id, Mutation.case_barcode AS case_barcode, Subject.id AS subject_id FROM gdc-bq-sample.dev.all_Subjects_v3_0_final AS Subject INNER JOIN UNNEST(Subject.ResearchSubject) AS _ResearchSubject INNER JOIN UNNEST(_ResearchSubject.Specimen) AS _Specimen LEFT JOIN UNNEST(_ResearchSubject.Diagnosis) AS _Diagnosis WHERE (((IFNULL(UPPER(_Diagnosis.stage), '') = UPPER(@stage_1)) OR (IFNULL(UPPER(_Diagnosis.stage), '') = UPPER(@stage_2))) AND (IFNULL(UPPER(_ResearchSubject.primary_diagnosis_site), '') = UPPER(@primary_diagnosis_site_1)))) as results WHERE rn = 1), researchsubject_files as (SELECT results.* EXCEPT(rn) FROM (SELECT ROW_NUMBER() OVER (PARTITION BY File.id, Subject.id, _ResearchSubject.id) as rn, File.id AS file_id, File.identifier AS file_identifier, File.label AS label, File.data_category AS data_category, File.data_type AS data_type, File.file_format AS file_format, File.associated_project AS file_associated_project, File.drs_uri AS drs_uri, File.byte_size AS byte_size, File.checksum AS checksum, File.data_modality AS data_modality, File.imaging_modality AS imaging_modality, File.dbgap_accession_number AS dbgap_accession_number, File.imaging_series AS imaging_series, '' AS specimen_id, _ResearchSubject.id AS researchsubject_id, Mutation.case_barcode AS case_barcode, Subject.id AS subject_id FROM gdc-bq-sample.dev.all_Subjects_v3_0_final AS Subject INNER JOIN UNNEST(Subject.ResearchSubject) AS _ResearchSubject LEFT JOIN UNNEST(_ResearchSubject.Diagnosis) AS _Diagnosis WHERE (((IFNULL(UPPER(_Diagnosis.stage), '') = UPPER(@stage_3)) OR (IFNULL(UPPER(_Diagnosis.stage), '') = UPPER(@stage_4))) AND (IFNULL(UPPER(_ResearchSubject.primary_diagnosis_site), '') = UPPER(@primary_diagnosis_site_2)))) as results WHERE rn = 1 AND CONCAT(results.file_id, results.subject_id, results.researchsubject_id) not in (SELECT CONCAT(specimen_files.file_id, specimen_files.subject_id, specimen_files.researchsubject_id) FROM specimen_files)), somatic_mutation_hg38_gdc_current_files as (SELECT results.* EXCEPT(rn) FROM (SELECT ROW_NUMBER() OVER (PARTITION BY File.id, Mutation.case_barcode) as rn, File.id AS file_id, File.identifier AS file_identifier, File.label AS label, File.data_category AS data_category, File.data_type AS data_type, File.file_format AS file_format, File.associated_project AS file_associated_project, File.drs_uri AS drs_uri, File.byte_size AS byte_size, File.checksum AS checksum, File.data_modality AS data_modality, File.imaging_modality AS imaging_modality, File.dbgap_accession_number AS dbgap_accession_number, File.imaging_series AS imaging_series, '' AS specimen_id, '' AS researchsubject_id, Mutation.case_barcode AS case_barcode, Subject.id AS subject_id FROM gdc-bq-sample.dev.all_Subjects_v3_0_final AS Subject LEFT JOIN UNNEST(Subject.ResearchSubject) AS _ResearchSubject LEFT JOIN UNNEST(_ResearchSubject.Diagnosis) AS _Diagnosis WHERE (((IFNULL(UPPER(_Diagnosis.stage), '') = UPPER(@stage_5)) OR (IFNULL(UPPER(_Diagnosis.stage), '') = UPPER(@stage_6))) AND (IFNULL(UPPER(_ResearchSubject.primary_diagnosis_site), '') = UPPER(@primary_diagnosis_site_3)))) as results WHERE rn = 1 AND CONCAT(results.file_id, results.case_barcode) not in (SELECT CONCAT(researchsubject_files.file_id, researchsubject_files.case_barcode) FROM researchsubject_files)), all_subjects_v3_0_final_files as (SELECT results.* EXCEPT(rn) FROM (SELECT ROW_NUMBER() OVER (PARTITION BY File.id, Subject.id) as rn, File.id AS file_id, File.identifier AS file_identifier, File.label AS label, File.data_category AS data_category, File.data_type AS data_type, File.file_format AS file_format, File.associated_project AS file_associated_project, File.drs_uri AS drs_uri, File.byte_size AS byte_size, File.checksum AS checksum, File.data_modality AS data_modality, File.imaging_modality AS imaging_modality, File.dbgap_accession_number AS dbgap_accession_number, File.imaging_series AS imaging_series, '' AS specimen_id, '' AS researchsubject_id, Mutation.case_barcode AS case_barcode, Subject.id AS subject_id FROM gdc-bq-sample.dev.all_Subjects_v3_0_final AS Subject LEFT JOIN UNNEST(Subject.ResearchSubject) AS _ResearchSubject LEFT JOIN UNNEST(_ResearchSubject.Diagnosis) AS _Diagnosis WHERE (((IFNULL(UPPER(_Diagnosis.stage), '') = UPPER(@stage_7)) OR (IFNULL(UPPER(_Diagnosis.stage), '') = UPPER(@stage_8))) AND (IFNULL(UPPER(_ResearchSubject.primary_diagnosis_site), '') = UPPER(@primary_diagnosis_site_4)))) as results WHERE rn = 1 AND CONCAT(results.file_id, results.subject_id) not in (SELECT CONCAT(somatic_mutation_hg38_gdc_current_files.file_id, somatic_mutation_hg38_gdc_current_files.subject_id) FROM somatic_mutation_hg38_gdc_current_files)),unioned_result as (SELECT specimen_files.* FROM specimen_files UNION ALL SELECT researchsubject_files.* FROM researchsubject_files UNION ALL SELECT somatic_mutation_hg38_gdc_current_files.* FROM somatic_mutation_hg38_gdc_current_files UNION ALL SELECT all_subjects_v3_0_final_files.* FROM all_subjects_v3_0_final_files) SELECT unioned_result.* FROM unioned_result"));
  }
